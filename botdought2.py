...
def get_weather_map(latitude, longitude):
    ...
    Uses tile coordinates based on latitude and longitude.
    """
    zoom = 10  # Adjust zoom level if necessary
    x_tile, y_tile = lat_lon_to_tile(latitude, longitude, zoom)

    # Choose the layer type: "clouds_new" is for cloud coverage, other layers are available (e.g., temperature, precipitation)
    map_layer = "clouds_new"  # You can change this to "precipitation_new", "temperature_new", etc.
    
    # Construct the map URL using calculated tile coordinates
    map_url = f"http://tile.openweathermap.org/map/{map_layer}/{zoom}/{x_tile}/{y_tile}.png?appid={WEATHER_TOKEN}"
    return map_url

def lat_lon_to_tile(lat, lon, zoom):
    """
    Converts latitude and longitude to OpenWeatherMap tile coordinates.
    """
    ...
    n = 2.0 ** zoom
    x_tile = (lon_rad + math.pi) / (2 * math.pi) * n
    y_tile = (1 - math.log(math.tan(lat_rad) + 1 / math.cos(lat_rad)) / math.pi) / 2 * n

    return int(x_tile), int(y_tile)

def fetch_weather_map(message, latitude, longitude):
    """
...
def fetch_weather_forecast(latitude, longitude, message):
    """
    Fetch the weather forecast for a given location and create buttons.
    """
    ...
    if forecast:
        ...
        for day in forecast['list'][::8]:  # Every 8th item represents a new day in the forecast (every 3 hours)
            ...
            # Create buttons with proper latitude and longitude in the callback data
...
def handle_weather_map_button(call):
    ...
    try:
        latitude = float(data[1])  # Extract latitude
        longitude = float(data[2])  # Extract longitude
    except ValueError:
        bot.send_message(call.message.chat.id, "Error: Invalid data for weather map.")
        return

    fetch_weather_map(call.message, latitude, longitude)
...
def handle_air_quality_button(call):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "Error: Invalid data for air quality.")
        return

    fetch_air_quality(call.message, latitude, longitude)
